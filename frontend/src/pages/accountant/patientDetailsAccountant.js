...
function PatientDetailsAccountant(props) {
  ...
  const [paymentStatus, setPaymentStatus] = useState(patientDetails.paymentStatus );
  ...
  const UpdatePatientPaymentStatus = () => {
    ...
    const body = {patId,paymentStatus,};
  ...
  const SetDataChangeFlag = () => {setDataChangedFlag(true);};


  useEffect(() => {
    getPatientsFromServer();
    GetPatientChargesFromServer();
    console.log("inside useEffect of patientDetails------>reception" +" dataflag ===>" +dataChangedFlag);
  }, [dataChangedFlag]);
    ...
    <div className="patientDetailsAccountant" >
      <div className="d-flex justify-content-between container-fluid" style={{ marginLeft: '50%', marginTop: '1%' }}>
        ...
        <div className="p-2 bd-highlight">
          <Button variant="warning" onClick={ToggleUpdateStatusModal} >Update Status</Button>
        </div>

        <div className="p-2 bd-highlight" style={{ marginLeft: '-10%' }} >
          <Button variant="secondary" onClick={() => {navigate("/accountant");}}>Goback </Button>
        </div>

        <div className="p-2 bd-highlight" style={{ marginRight: '20%' }}>
          <Button variant="danger" onClick={() => { navigate("/signIn"); }} >Logout </Button>
        </div>

      </div>

      <table className="patientDetailsAccountantTable table table-hover" style={{ marginLeft: '50%' }}>
        <tbody>
          <tr>
            <th>Name</th>{" "}
          ...
          <tr>
            ...
            <td>{patientDetails.doctorFirstName +" " +patientDetails.doctorLastName}</td>
          </tr>
          <tr>
      ...
      <table className="chargesTable table table-hover " style={{ marginLeft: '50%' }}>
        <tr>
          <th>Sr.no</th>
          <th>Charges type</th>
          <th>Charges(Rs.)</th>
        ...
        <tr >
          ...
          <td style={{ backgroundColor: "lightgreen", fontWeight: "bold", fontSize: "larger" }}>
            Rs.{patientCharges.wardCharges +patientCharges.medicineCharges +patientCharges.doctorCharges}
          </td>
        </tr>
      </table>
      ...
      <table className="chargesTable table table-hover"style={{ marginLeft: '50%' }} >
        <tr>
          <th>Total Bill :</th>
          <th>Rs.{patientCharges.wardCharges +patientCharges.medicineCharges +patientCharges.doctorCharges}</th>
        </tr>
        <tr>
