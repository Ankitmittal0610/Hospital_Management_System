...
function AddNewPatientModal(props) {
  ...
  const [uniqueBedAlloted, setUniqueBedAlloted] = useState("");
      ...
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          <div className="form-group needs-validation">
            ...
            {firstName.length < 3 ? (
              <h6 className="emptyFieldWarning">*cannot be empty and lenght greater than 3</h6>
              ) : ( <div></div>)}
          </div>
          {/* ========================================================= */}
          <div className="form-group needs-validation">
            <label className="form-label">Last Name</label>
            <input onChange={(e) => {setLastName(e.target.value);}}type="text"className="form-control"placeholder="patient last name"required/>
            {lastName.length < 3 ? ( <h6 className="emptyFieldWarning">*cannot be empty and lenght greater than 3</h6>
            ) : (<div></div>)}
          </div>
          {/* ===============================email===================================== */}
          <div className="form-group needs-validation">
            <label className="form-label">email</label>
            <input onFocus={CheckForEmailDupicacyOnServer} onBlur={CheckForEmailDupicacyOnServer} onChange={(e) => { setUniqueEmail(""); setEmail(e.target.value); CheckForEmailDupicacyOnServer();}} type="email" className="form-control"/>
            {email.match(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/) &&
             email.endsWith(".com") ? ( <div className=""> </div>
          ...
          <div>
            ...
            <input onDrop={DecreaseBedCount} onBlur={() => { CheckForBedDupicacyOnServer();
                if (wardFullFlag == false) { IncreaseBedCount(); } }}
              onChange={(e) => {
          ...
          <div>
            {securityQuestion == "select security question" ? (
              <h6 className="emptyFieldWarning">* security question cannot be empty</h6>
            ) : (<div></div>
            )}
            <DropdownButton size="sm" title={securityQuestion} style={{ marginTop: "10px" }} variant="warning">
              <Dropdown.Item as="Button">
                <div onClick={(e) => { setSecurityQuestion(e.target.innerText);}}>my pet name</div>
              </Dropdown.Item>
            </DropdownButton>
          </div>
          {/* ========================================security answer============================================== */}
          <div>
            ...
            <input onChange={(e) => {setSecurityAnswer(e.target.value); }} type="text" className="form-control" placeholder="enter your pet name" required />
            {securityAnswer == "" ? (
          ...
          <div>
            ...
            <input onChange={(e) => {setPatientProblem(e.target.value);}} type="text-area" className="form-control"required />
            {patientProblem.length < 5 ? (
              <h6 className="emptyFieldWarning">*problem cannot be empty min length 5{" "}</h6>
            ) : (<div></div>)}
          </div>
        </Modal.Body>

        <Modal.Footer>
          <Button variant="secondary" onClick={ModalHandling}> close</Button>
