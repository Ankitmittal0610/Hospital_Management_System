...
const AddWardModal = (props) => {
  ...
  const [token, setToken] = useState(sessionStorage.getItem("token_admin"));
    ...
    const addWard=()=>{
      ...
      const body = {type,charges,maxCapacity};
      ...
      axios.post(url, body).then((res) => {
        ...
        }else{toast.warning("Ward not Added")}
      }).catch(err=>{
        navigate("/error");
    });
      ...
      <div className="">
        <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
          ...
          <div className="form-group needs-validation">
              ...
              {/* empid validation */}
              {type == "" ? (
                <h6 className="emptyFieldWarning"> *ward type cannot be empty</h6>
              ) : (
                <div></div>
              )}
            </div>
            ...
            <div className="form-group needs-validation">
              ...
              <input onChange={  (e)=>{setCharges(e.target.value)}}type="number"className="form-control"/>
              {/* empid validation */}
              {charges == 0 ? (
                <h6 className="emptyFieldWarning">*ward charges cannot be empty</h6>
              ) : (
                <div></div>
              )}
            </div>
            ...
            <div className="form-group needs-validation">
              ...
              <input onChange={ (e)=>{setMaxCapacity(e.target.value)}}type="number"className="form-control"/>
              {/* empid validation */}
              {maxCapacity == 0 ? (
                <h6 className="emptyFieldWarning">*ward max capacity cannot be empty</h6>
              ) : (
                <div></div>
              )}
            </div>
          ...
          <Modal.Footer>
            <div style={{ position: "relative", left: "-120px" }}>
              <Button size="sm" variant="warning" onClick={()=>{setAddWardModalFlag(false); setShow(false)}}>Go back</Button>
            </div>
  
            <Button size="sm" variant="success" onClick={addWard} >Add Ward </Button>
          </Modal.Footer>
        </Modal>
      </div>
    );
